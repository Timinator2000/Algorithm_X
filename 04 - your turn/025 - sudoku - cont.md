...
The following 3 options always leave Algorithm X in charge. What does that mean. In some way, we are restricting what Algorithm X sees as possible. If there is only a single option available for one of the Sudoku cells, then Algoritm X has no choice but to use that option.
