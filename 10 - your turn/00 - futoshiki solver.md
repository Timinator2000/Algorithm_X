...
__Test Case 2: Comparisons only horizontal__ and __Test Case 3: Comparisons only vertical__ are the only two interesting test cases you can completely solve simply by reducing the problem space based on inqualities. However, this one small reduction effort puts Algorithm X in position to find solutions to __all__ remaining test cases very fast.
