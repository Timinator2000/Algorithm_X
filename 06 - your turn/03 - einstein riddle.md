...
If `a` and `c` must be chosen together, I could shift my perspective and say that `a` _must not_ be chosen with `b`, `a` _must not_ be chosen with `d` and `a` _must not_ be chosen with `e`. The only options left are to choose both `a` and `c` or to choose some combination of  `b`, `d` and `e`.
